...
class Model():
    ...
    def __init__(self):
        ...
        self.variance=[0,5,10,15,20,25]
        self.out=np.zeros((self.Nz,self.Nx))
    ...
    def bruit(self):
        ...
        while(i!=6):
            ...
            bruit= np.random.randn(ligne,colonne)* self.variance(i)# Gaussian noise : mean (0) and standard deviation (10)
...
class View():
    def __init__(self, master):
        ...
        self.cbar=(self.ax0).imshow(Model().out,extent=[Model().xMin, Model().xMax,Model().zMax ,Model().zMin],
                                        aspect='auto',cmap='jet') #axe colormap
        
        self.fig.colorbar(self.cbar,orientation='vertical')

        

        #self.fig2,self.ax1 = plt.subplots( nrows=1 ,figsize=(6,6))
        
        #self.cbar2=(self.ax1).imshow(Model().out,extent=[Model().xMin, Model().xMax,Model().zMax ,Model().zMin],
                                        #aspect='auto',cmap='jet') #axe colormap
        
        #self.fig2.colorbar(self.cbar2, ticks=[0,50,100],orientation='vertical')
...
##        self.root.refresh()
    ...
    def my_plot(self,event):
        ...
        #self.view.ax0.contourf(self.model.res2["imagenorm"],extent=[Model().xMin, Model().xMax,Model().zMin ,Model().zMax])
        
        self.model.bruit()
        self.view.ax0.contourf(self.model.res3["imagebruit"],extent=[Model().xMin, Model().xMax,Model().zMin ,Model().zMax])
        #self.view.axf.clear()
        self.view.fig.canvas.draw()
